...
// Function to check if matricula, CNPJ, or email already exists
export const checkCondominiumExists = async (matricula: string, cnpj: string, email: string) => {
  try {
    ...
    if (matriculaError && matriculaError.code !== 'PGRST116') { // PGRST116 = no rows returned
      console.error('Error checking matricula:', matriculaError);
      throw matriculaError;
    }
    
    // Check if CNPJ already exists (only if CNPJ is provided)
    ...
    return {
      matriculaExists: !!matriculaData,
      cnpjExists,
      emailExists: !!emailData
    };
  } catch (error) {
    console.error('Error in checkCondominiumExists:', error);
    throw error;
  }
};
