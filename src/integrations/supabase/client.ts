...
const compareCondominiumChanges = (oldData: Condominium, newData: Condominium): FieldChange[] => {
  ...
  Object.keys(newData).forEach(key => {
    ...
    // Trata o campo ativo de forma especial para mostrar "Ativo" ou "Inativo" em vez de true/false
    if (key === 'ativo') {
      if (oldData[key as keyof Condominium] !== newData[key as keyof Condominium]) {
        changes.push({
          field: fieldDisplayNames[key] || key,
          oldValue: oldData[key as keyof Condominium] ? 'Ativo' : 'Inativo',
          newValue: newData[key as keyof Condominium] ? 'Ativo' : 'Inativo'
        });
      }
      return;
    }
